...
/******************************
	-	BUILD LOADER 	-
********************************/
function buildLoader(container,opt) {
		// CREATE THE SPINNER
		if (container.find('.esg-loader').length>0) return false;
		container.append('<div class="esg-loader '+opt.spinner+'">'+
								  		'<div class="dot1"></div>'+
								  	    '<div class="dot2"></div>'+
								  	    '<div class="punchgs.Bounce1"></div>'+
										'<div class="punchgs.Bounce2"></div>'+
										'<div class="punchgs.Bounce3"></div>'+
									 '</div>');

		if (opt.spinner=="spinner1" || opt.spinner=="spinner2") container.find('.esg-loader').css({backgroundColor:opt.spinnerColor});
		if (opt.spinner=="spinner3" || opt.spinner=="spinner4") container.find('.punchgs.Bounce1, .punchgs.Bounce2, .punchgs.Bounce3, .dot1, .dot2').css({backgroundColor:opt.spinnerColor});
		punchgs.TweenLite.to(container,0.3,{minHeight:"100px",zIndex:0});
...
*/
			...
			****************************************/
			jQuery(opt.filterGroupClass+'.esg-filter-wrapper,'+opt.filterGroupClass+' .esg-filter-wrapper').each(function() {
				var efw = jQuery(this);
				//efw.addClass("dropdownstyle");
				if (efw.hasClass("dropdownstyle")) {
					/*efw.wrapInner('<div class="esg-dropdown-wrapper"></div>');

					efw.prepend('<div class="esg-selected-filterbutton"><span>Filter DropDown</span><i class="eg-icon-down-open"></i></div>');
					*/
					...
					if (!is_mobile()) {
						efw.hover(function() {
							...
							//punchgs.TweenLite.to(efw.find('.eg-icon-down-open'),0.3,{rotationZ:-180});
							efw.find('.esg-dropdown-wrapper').stop().slideDown(200);


						},function() {
							var efw = jQuery(this).closest('.esg-filter-wrapper');
							efw.find('.esg-selected-filterbutton').removeClass("hoveredfilter");
							//punchgs.TweenLite.to(efw.find('.eg-icon-down-open'),0.3,{rotationZ:0});
							efw.find('.esg-dropdown-wrapper').stop().slideUp(200);
			...
			*************************************************/
			jQuery(opt.filterGroupClass+'.esg-left,'+opt.filterGroupClass+' .esg-left').click(function() {
			...
			***********************************************/
			jQuery(opt.filterGroupClass+'.esg-right,'+opt.filterGroupClass+' .esg-right').click(function() {
				opt = getOptions(container);
				opt.oldpage = opt.currentpage;
				opt.currentpage++;
 ...
 **********************************/
 function prepareItemsInGrid(opt,appending) {
	...
	loadVideoApis(container,opt);
 	...
 	//Show Container
 	if (!appending) {
	 	punchgs.TweenLite.fromTo(ul,0.2,{autoAlpha:0,opacity:0},{force3D:"auto",autoAlpha:1,ease:punchgs.Power3.easeInOut,clearProps:"transform"});
	 	punchgs.TweenLite.fromTo(esgo,0.2,{autoAlpha:0,opacity:0},{autoAlpha:1,ease:punchgs.Power3.easeInOut,clearProps:"transform"});
...
var waitForLoads = function(element,opt) {
	...
	if (!element.hasClass("loadedmedia") && opt.layout!="even") 
...
function runGrid(opt,newelementadded) {
		...
		if (opt.firstshowever==undefined) {
			if (container.is(":hidden")) {
				punchgs.TweenLite.fromTo(container,0.3,{autoAlpha:0,display:"block"},{autoAlpha:1,display:"block"});
				setTimeout(function() {

//(function() {
	...
	jQuery('body').on('click', '#ess-grid-add-custom-shortcode, #ess-grid-add-custom-shortcode-special', function(){
		...
		if(jQuery('select[name="ess-grid-tiny-existing-settings"] option:selected').val() !== '-1'){
			...
			if(ess_grid_is_vc)
				jQuery('#esg-vc-input-alias').val(jQuery('select[name="ess-grid-tiny-existing-settings"] option:selected').val());
			...
			if(typeof(settings_raw['ess-grid-tiny-max-entries']) !== 'undefined' && esg_create_by_predefined !== false){
				...
				if(ess_grid_is_vc)
					jQuery('#esg-vc-input-settings').val(settings.replace(/\"/g, "'"));
			}
		}else{
		...
		if(jQuery(this).attr('id') == 'ess-grid-add-custom-shortcode'){
			...
			if(ess_grid_is_vc)
				jQuery('#esg-vc-input-layers').val(layers.replace(/\"/g, "'"));
				
		}else{ //add special stuff here
			content += " special='"+esg_create_by_predefined+"'";
			if(ess_grid_is_vc)
				jQuery('#esg-vc-input-special').val(esg_create_by_predefined);
		}
		...
		if(!ess_grid_is_vc){
			...
			//jQuery('#esg-vc-input-content').val(content);
			jQuery('#ess-grid-tiny-mce-dialog').dialog('close');
			jQuery('.vc_panel-btn-save').click();
		...
		jQuery('body').on('click', '#eg-add-predefined-grid', function(){
			...
			if(grid_handle !== '-1'){
				...
				if(!ess_grid_is_vc){
					...
					jQuery('#esg-vc-input-alias').val(grid_handle);

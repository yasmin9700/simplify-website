...
var GridEditorEssentials = new function(){
	...
	t.set_inline_styles = function(on_layer, settings, demo_element){
		...
		if(on_layer.data('handle') == 'eg-line-break') return true;
	...
	t.propagate_element_selector = function(){
		...
		if(selected_layer !== null){
			...
			jQuery('.eg-element-class-setter').text(eg_lang.class_name+' .eg-'+jQuery('#eg-item-skin-slug').text()+'-element-'+selected_layer.data('id'));
	...
	t.set_default_elements_draggable = function(){
		jQuery("#skin-dz-tl, #skin-dz-br, #skin-dz-c, #skin-dz-m, .skin-dz-elements, .eg-special-element").disableSelection();

		jQuery(".mainul .skin-dz-elements, .eg-special .eg-special-element").draggable({
